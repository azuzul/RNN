...
class RNN:
    def __init__(self, vocab_size, hidden_size=100, sequence_length=30, learning_rate=1e-1):
        ...
        self.b = np.zeros(hidden_size)  # input bias H x 1
    ...
    def rnn_step_forward(self, x, h_prev, U, W, b):
        ...
        h_current = np.tanh(np.dot(h_prev, W) + np.dot(x, U) + b)   # BS x H
    ...
    def rnn_step_backward(self, grad_next, cache):
        ...
        W, x, h_prev = cache
        dh_prev, dU, dW, db = np.dot(grad_next, W), np.dot(x.T, grad_next), np.dot(grad_next.T, h_prev), np.sum(grad_next, axis=0)
    ...
    def rnn_backward(self, dh, cache):
        ...
        for t in range(self.sequence_length-1, 0, -1):
            ...
            dh_curr = dh[t] + np.dot(1 - h_curr**2, self.W)  # trenutni grad + upstream * dht/dat * dat/dgt-1
    ...
    def output_loss_and_grads(self, h, V, c, y):
        ...
        for t in range(self.sequence_length):
            ...
            yp = y[:, t, :]
            dO = s - yp  # BS x leng
            dV += np.dot(hp.T, dO)  # ( H x BS ) * ( BS x leng ) = H x leng
            dc += np.sum(dO, axis=0)  #
            dh.append(np.dot(dO, self.V.T))  # ( BS x leng ) * ( leng x H ) = ( BS x H )
